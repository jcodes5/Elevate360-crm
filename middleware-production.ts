...
function addSecurityHeaders(response: NextResponse, request: NextRequest) {
  ...
  if (config.security.rateLimitEnabled) {
    response.headers.set("X-RateLimit-Limit", config.security.apiRateLimit.requests.toString());
  }
  ...
  try {
    ...
    if (refreshToken) {
      try {
        ...
        try {
          const newPayload = ProductionAuthService.verifyAccessToken(newTokenData.accessToken);
