...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const updated = await prisma.campaign.update({ where: { id: params.id }, data: { targetAudience: body } })
    return NextResponse.json({ success: true, data: updated, message: 'Audience updated' } satisfies ApiResponse)
  } catch { return NextResponse.json({ success: false, message: 'Failed to update audience' } satisfies ApiResponse, { status: 500 }) }
}
