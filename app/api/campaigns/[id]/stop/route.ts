...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { payload } = auth
  try {
    const updated = await prisma.campaign.update({ where: { id: params.id }, data: { status: 'COMPLETED', completedAt: new Date(), updatedAt: new Date() } })
    if (updated.organizationId !== payload.organizationId) return unauthorized('Forbidden', 'FORBIDDEN', 403)
    return NextResponse.json({ success: true, data: updated, message: 'Campaign stopped' } satisfies ApiResponse)
  } catch { return NextResponse.json({ success: false, message: 'Failed to stop campaign' } satisfies ApiResponse, { status: 500 }) }
}
